...
export class RoomRepository {
    async create(data: Omit<IRoom, 'id' | "inviteCode" | "inviteCodeExpiresAt" | 'createdAt' | 'updatedAt'>) {
    ...
    async findById(id: string) {
        ...
        }) as Promise<IRoomWithRelations | undefined>;
    }

    async findByInviteCode(code: string) {
        return db.query.roomsSchema.findFirst({
            where: and(
                eq(roomsSchema.inviteCode, code),
                eq(roomsSchema.isPublic, true)
            ),
            with: { members: true }
        });
    }
    ...
    async update(id: string, data: Partial<IRoom>) {
        const [room] = await db.update(roomsSchema)
