...
export class RoomWithRelationsDto implements IRoomWithRelations {
  ...
  constructor(model: IRoomWithRelations) {
    this.id = model.id;
    ...
    this.members =
      model.members?.map((m: IMemberWithPermissions) => new RoomMemberWithPermissionsDto(m)) || [];
    this.statuses = model.statuses?.map((s: IStatus) => new StatusDto(s)) || [];
    this.tasks = model.tasks?.map((t: ITask) => new TaskDto(t)) || [];
  }
}
